 ...
 */
export async function uploadProductImage(file: File): Promise<string | undefined> {
    ...
    try {
        ...
        return fileName as string; // Or return a URL if serving via a web server
    } catch (error) {
...
export const deleteProductImage = (imageUrl: string) => {
    ...
    const imagePath = path.join(process.cwd(), "public", "imgs", imageUrl);
